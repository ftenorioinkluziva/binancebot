...
export default function NewStrategyPage() {
  ...
  const DCAForm = useForm<StrategyFormValues>({
  ...
  const BollingerForm = useForm<StrategyFormValues>({
    resolver: zodResolver(strategySchema),
    defaultValues: {
      type: 'BollingerBands',
      active: false,
  ...
  const MAForm = useForm<StrategyFormValues>({
    resolver: zodResolver(strategySchema),
    defaultValues: {
      type: 'MovingAverage',
      active: false,
  ...
  const onSubmit = async (data: StrategyFormValues) => {
    ...
    try {
      // Garantir que a moeda está definida em todas as estratégias
      const dataWithCurrency = {
        ...data,
        currency: data.currency || (data.type === 'DCA' ? 'BRL' : 'USDT')
      };
      
      await StrategyService.createStrategy(dataWithCurrency);
